...
$sql="SELECT DISTINCT Doctor FROM a_350215159126";
 ...
 function history_show_city()
    {
      if (window.XMLHttpRequest) {
        ...
        xmlhttp.open("POST","history_shpw_list.php?sdate="+document.getElementById('sy').value+"-"+document.getElementById('sm').value+"-"+document.getElementById('sd').value+"&edate"+document.getElementById('ey').value+"-"+document.getElementById('em').value+"-"+document.getElementById('ed').value+"&doctor="+document.getElementById('doctor').value,true);
      xmlhttp.send(); 
    }
</script>
<input type="button" value="SUBMIT" onclick="history_show_city()">
...
$conn=new mysqli($servername,$username,$password,$dbname);
$sql="SELECT DISTINCT  Doctor FROM  ORDER BY Sno DSC";
$result=$conn->query($sql);
echo "<table>
          <tr>
            <th>Date</th>
